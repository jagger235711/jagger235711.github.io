...
- ffn实际上就是一个全连接 卷积层-relu-卷积层
    ...
    是的，你理解得已经很接近了 👍
    我们可以更精确一点来看 **FFN 在 Transformer 里的作用**：
  ...
  - Self-Attention 是一个**线性加权求和**机制，本身是线性的。
  ...
  - FFN 则在「单个 token 内部」进行**维度提升和降维**（比如从 $d$ → $4d$ → $d$），让每个 token 的表示更丰富。
...
![20250820112729](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820112729.png)

- 目的是想和 CV 一样做微调
![20250820114810](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820114810.png)

- 就是一个只有编码器的 Transformer
![20250820185331](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820185331.png)

- 输入变成两个句子拼接。 QA 等 NLP 任务通常是
- 做分隔区分
![20250820185840](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820185840.png)

- BERT 是双向的。 文本预测时要求从前往后一个一个预测，所以没办法直接使用
![20250820190308](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820190308.png)

![20250820191622](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820191622.png)

- 数据量下限是 1B 。数据量太少别用 BERT
![20250820191754](https://cdn.jsdelivr.net/gh/jagger235711/coooool@main/img/20250820191754.png)
...
- **预训练阶段**
  ...
  - BERT 的产出是 **一个模型**（不是两个）。
  - 在训练时，这个模型会同时优化 **MLM 任务**和 **NSP 任务**。
...
所以一句话总结：
👉 **BERT 预训练的结果 = 一个单一模型，它的参数是在 MLM + NSP 的双任务训练下学到的**。
...
### 操作步骤
  ...
  1. **确定少数类样本**：首先需要明确数据集中的少数类，即样本数量较少的类别。
  2. **计算距离**：对于每个少数类样本，计算它与其他少数类样本之间的距离（一般使用欧氏距离等度量方式），然后根据距离确定每个少数类样本的k个最近邻（k是一个超参数，需要根据实际情况进行设置）。
  3. **生成合成样本**：对于每个少数类样本，从它的k个最近邻中随机选择一个或多个邻居，在该样本和选中的邻居之间随机生成新的样本点。例如，对于少数类样本 $x_i$，选择它的一个最近邻 $x_j$，通过以下公式生成合成样本 $x_{new}$：
  \[ x_{new} = x_i + \delta \times (x_j - x_i) \]
  其中，$\delta$ 是一个在 $[0, 1]$ 之间的随机数。通过多次执行这个过程，可以生成多个新的少数类样本，增加少数类样本在数据集中的占比。
...
### 使用SMOTE方法的步骤
  ...
  1. **数据准备**
  将数据集划分为特征矩阵 `X` 和标签向量 `y` 。特征矩阵 `X` 包含每笔交易的相关信息，比如交易金额、交易时间、交易地点、持卡人近期消费习惯等多个特征；标签向量 `y` 表示交易是否为欺诈，1代表欺诈交易，0代表正常交易。

  2. **导入库并实例化SMOTE对象**
  在Python中，可以使用 `imblearn` 库来实现SMOTE方法，代码如下：
  ...
  在这一步中，`SMOTE` 算法会分析少数类（欺诈交易）样本的分布情况。比如，对于每一个欺诈交易样本，它会找到其 `k` 个最近邻（假设设置 `k=5` ），然后在该样本和其最近邻样本之间随机生成新的样本点。最终，得到经过过采样后的特征矩阵 `X_resampled` 和标签向量 `y_resampled` ，此时欺诈交易样本的数量会增加，数据集的类别不平衡程度得到缓解。

  4. **训练模型**
  使用经过SMOTE处理后的数据集来训练分类模型，比如逻辑回归模型：
...
- 感觉主要还是对目标测试集的分布不确定，这次的挑战就是在 imbalanced的 train set，以及小样本上。 下周也去试下RCNN好了，单纯精度上来看 RCNN也许会更好 :D
